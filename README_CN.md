...
- Gemini 思考期间持续发送心跳
- 在 Gemini 最后一个可见完成帧结束响应，不等待空闲尾流
- Gemini 私有富组件标签自动转换为普通 Markdown
- 空上游回复自动重试
...
- Gemini 生成图片和网页图片在本地缓存后以 Markdown 图片内容返回
...
| `media_store_path` | Gemini 输出图片缓存目录 | Docker 中为 `/app/data/media` |
| `media_store_ttl_sec` | 输出图片保留时间 | `86400` 秒 |
| `public_base_url` | 图片链接使用的公网地址 | 为空时从反代请求头推断 |
| `reuse_upstream_sessions` | 复用 Gemini Web metadata | 默认关闭 |
| `upstream_session_backend` | 首选上游会话后端 | `gemini_webapi`；direct 传输作为回退 |
| `webapi_fast_finish` | 隐藏 Gemini Web 的空闲响应尾流 | 默认开启；尾流会在后台完成 |
...
- 返回图片会通过登录态下载并从 `/media/*` 提供；请持久化 `/app/data`，若反代请求头无法体现公网 HTTPS 地址，请配置 `public_base_url`。
